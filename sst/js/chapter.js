...
(function () {
  ...
  const subColor = { hist: '#ff5d7a', geo: '#34e0a1', pol: '#7c6cff', eco: '#ffb24d' }[C.sub];
  const subName = { hist: 'History', geo: 'Geography', pol: 'Political Science', eco: 'Economics' }[C.sub];
  ...
  if (heroCard) heroCard.classList.add({ hist: 'c-hist', geo: 'c-geo', pol: 'c-pol', eco: 'c-eco' }[C.sub]);
  $('#ch-eyebrow').textContent = subName + ' · Chapter ' + (C.sub === 'hist' ? C.no : C.sub === 'geo' ? C.no - 4 : C.sub === 'pol' ? C.no - 9 : C.no - 13);
  ...
  notes.forEach((p, i) => {
    ...
    el.className = 'lb reveal' + (C.sub === 'hist' ? ' lb-pink' : C.sub === 'geo' ? ' lb-emerald' : C.sub === 'pol' ? ' lb-violet' : ' lb-amber');
    ...
    lbSec.innerHTML =
      '<div class="container">' +
        '<div class="sec-head reveal">' +
          ...
          '<p class="sec-lede">The complete NCERT text of this chapter, in order \u2014 each block followed by its plain-English explanation, exactly like the <a href="pol3.html#overview" style="color:var(--cyan)">Gender, Religion &amp; Caste</a> guide.</p>' +
        '</div>' +
  ...
  /* ---------- prev / next ---------- */
  const order = [].concat(window.SUBJECTS.hist.chapters, window.SUBJECTS.geo.chapters, window.SUBJECTS.pol.chapters, window.SUBJECTS.eco.chapters);
  const i = order.indexOf(C.slug);
  const prev = order[i - 1], next = order[i + 1];
  if (prev) {
    $('#prev-link').href = prev + '.html';
    $('#prev-title').textContent = window.CHAPTERS[prev].title;
  } else $('#prev-nav').style.display = 'none';
  if (next) {
    $('#next-link').href = next + '.html';
    $('#next-title').textContent = window.CHAPTERS[next].title;
  } else $('#next-nav').style.display = 'none';
})();
